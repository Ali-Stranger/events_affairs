...
class AdminStore extends ChangeNotifier {
  ...
  /// vendorId -> selected primary service
  final Map<String, String> vendorPrimaryService = {};
  ...
  Future<void> initialize() async {
    try {
      ...
      // // Seed local hardcoded vendors
      // for (final v in allVendors) {
      //   vendorStatus.putIfAbsent(
      //     v.name,
      ...
      // if (allVendors.isNotEmpty) {
      //   vendorStatus[allVendors.first.name] =
      //       VendorStatus.pending;
      // }

      // if (allVendors.length > 2) {
      //   vendorStatus[allVendors[2].name] =
      //       VendorStatus.pending;
      // }
      ...
      debugPrint('AdminStore initialize error: $e');
    } finally {
      isLoading = false;
      notifyListeners();
    }
  ...
  int get totalVendors => firestoreVendors.length;
}
